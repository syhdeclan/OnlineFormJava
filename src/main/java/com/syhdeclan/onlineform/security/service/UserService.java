...
public class UserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (jwtUserList.size() != 1) {
            throw new WebException(Code.USER_OR_PASS_WRONG);
        }
        JwtUser jwtUser = jwtUserList.get(0);
        if (!jwtUser.isEnabled()) {
            throw new WebException(Code.USER_NON_ACTIVED);
        }
        //返回的这个对象是要干嘛用的
    ...
    public boolean checkEmailIsUsed(String email){
        return this.userRepository.findByPhoneAndIsDelete(email,0).size() == 1;
    }
